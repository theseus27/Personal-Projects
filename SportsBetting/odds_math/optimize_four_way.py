...
#TEST_AMOUNTS = [i for i in range(51)] # $1 intervals
TEST_AMOUNTS = [1, 2, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
...
def optimize_avg_profit_4_way(am_odds, probs):
        ...
        for j in range(len(TEST_AMOUNTS)):
            for k in range(len(TEST_AMOUNTS)):
                for m in range(len(TEST_AMOUNTS)):
                    amounts = [TEST_AMOUNTS[i], TEST_AMOUNTS[j], TEST_AMOUNTS[k], TEST_AMOUNTS[m]]

                    # Check that there is a minimum bet on minimum odds
                    valid = False
                    for n in range(len(am_odds)):
                        if am_odds[n] >= MIN_ODDS and amounts[n] >= MIN_BET:
                            valid = True
                    if not valid:
                        continue
    ...
    return optimal_bets, max_profit
...
def even_distro_profits_4_way(am_odds, probs):
    ...
    return optimal_bets, smallest_diff
...
def best_of_both_4_way(am_odds, probs, max_prof, even_prof):
    ...
    for i in range(len(len(TEST_AMOUNTS))):
        for j in range(len(TEST_AMOUNTS)):
            for k in range(len(TEST_AMOUNTS)):
                for m in range(len(TEST_AMOUNTS)):
                    amounts = [TEST_AMOUNTS[i], TEST_AMOUNTS[j], TEST_AMOUNTS[k], TEST_AMOUNTS[m]]

...
public class Aspiration implements AI {

    private boolean timeToStop;
    private Evaluator evaluator = new StandardEval();
    ...
    public Response getBestResponse(Board board, int depth) throws OutOfTimeException {
        ...
        int window = 10;
        while (true) {
            int alpha = evaluator.getEvaluation(board) - window;
            int beta = evaluator.getEvaluation(board) + window;
            response = getResponse(board.copy(), new Response(), depth, alpha, beta);
